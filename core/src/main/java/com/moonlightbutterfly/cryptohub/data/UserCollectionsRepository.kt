...
class UserCollectionsRepository(
    ...
    fun getCollection(name: String) = if (userDataSource.isUserSignedIn().getOrThrow()) {
        userCollectionsRemoteDataSource.getCollection(userDataSource.getUser().getOrThrow().userId, name)
    } else {
        userCollectionsLocalDataSource.getCollection(name)
    }

    fun getAllCollectionNames() = if (userDataSource.isUserSignedIn().getOrThrow()) {
        userCollectionsRemoteDataSource.getAllCollectionNames(userDataSource.getUser().getOrThrow().userId)
    } else {
    ...
    suspend fun clearCollection(name: String) = if (userDataSource.isUserSignedIn().getOrThrow()) {
        userCollectionsRemoteDataSource.clearCollection(userDataSource.getUser().getOrThrow().userId, name)
    } else {
    ...
    suspend fun createCollection(name: String) = if (userDataSource.isUserSignedIn().getOrThrow()) {
        userCollectionsRemoteDataSource.createCollection(userDataSource.getUser().getOrThrow().userId, name)
    } else {
    ...
    suspend fun removeCollection(name: String) = if (userDataSource.isUserSignedIn().getOrThrow()) {
        userCollectionsRemoteDataSource.removeCollection(userDataSource.getUser().getOrThrow().userId, name)
    } else {
        userCollectionsLocalDataSource.removeCollection(name)
    }

    suspend fun addToCollection(asset: CryptoAsset, collectionName: String) = if (userDataSource.isUserSignedIn().getOrThrow()) {
        userCollectionsRemoteDataSource.addToCollection(userDataSource.getUser().getOrThrow().userId, asset, collectionName)
    } else {
        userCollectionsLocalDataSource.addToCollection(asset, collectionName)
    }

    suspend fun removeFromCollection(asset: CryptoAsset, collectionName: String) = if (userDataSource.isUserSignedIn().getOrThrow()) {
        userCollectionsRemoteDataSource.removeFromCollection(userDataSource.getUser().getOrThrow().userId, asset, collectionName)
    } else {
